...
description: Defensive coding at write-time — the secure-by-default patterns (input validation, output encoding, parameterization, authn/authz, crypto defaults, secret handling, dependency hygiene) applied while code is being written, not after. Complements `core/skills/security-audit` (scoped review) and `core/agents/security-reviewer` (review-as-a-subagent).
source: core
triggers: /secure-code, secure coding, input validation, output encoding, parameterized query, OWASP, SSRF, XSS, CSRF, SQLi, JWT, OAuth, password hashing, argon2, bcrypt, secrets, TLS, CORS, CSP
ported-from: https://github.com/Jeffallan/claude-skills/tree/main/skills/secure-code-guardian
...
Secure-by-default guidance **at write time**. Applied proactively
while code is being authored, not as a separate audit pass. Activates
when the work touches trust boundaries, auth, crypto, secrets, or
third-party integrations.

> **See also:**
>
> - `core/skills/security-audit/` — scoped security review
> - `core/agents/security-reviewer.md` — subagent lens for reviews
> - `core/rules/security.md` — baseline always-on rules
> - `stacks/node-ts/skills/typescript-pro/references/runtime-validation.md`
>   — Zod/Valibot at trust boundaries
> - `stacks/kubernetes/skills/kubernetes-specialist/references/security.md`
...
- [`references/input-and-output.md`](references/input-and-output.md)
  — trust boundaries, schema validation, parameterized queries,
  output encoding by context, SSRF defense, deserialization, file
  upload handling.
- [`references/authn-and-authz.md`](references/authn-and-authz.md) —
  password storage (argon2id, bcrypt), session vs. JWT vs. opaque
  tokens, OAuth/OIDC defaults, authorization checks at the right
  layer, tenant isolation patterns, rate limiting.
- [`references/crypto-and-secrets.md`](references/crypto-and-secrets.md)
  — "don't roll your own crypto", choosing libraries (libsodium,
  WebCrypto, `cryptography`), at-rest encryption keys, secret
  storage (Vault, AWS SM, sealed secrets), secret scanning, signing
  webhooks, TLS discipline.

## Core workflow

1. **Identify the trust boundary.** What crosses from attacker-
   controllable to trusted? Every such crossing needs validation.
2. **Validate on input, encode on output.** Parse with a schema at
   the edge, re-encode when it leaves for a new context (HTML, SQL,
   shell, headers).
3. **Prefer parameterization / structured APIs** over string
   concatenation — SQL params, prepared statements, `subprocess`
   with a list, HTML templates with auto-escape.
4. **Fail closed.** When in doubt, deny. `401` > "guess it's fine".
5. **Least privilege.** Tokens scope-limited, DB users minimal,
   service accounts per function.
6. **Keep secrets out of source**, out of logs, out of error pages,
   off the command line. Rotate them on a schedule and on incidents.
7. **Update dependencies routinely.** Transitive vulnerabilities
   are the majority of real-world breaches.
...
- **Trusting the client.** Hidden fields, `disabled=true`, client-
  side validation — all gone once the attacker opens devtools.
...
- **Authorization checks in middleware only.** Per-resource checks
  at the data layer too. Tenant isolation bugs hide here.
- **Sensitive data in logs.** Passwords, tokens, PII, payment info.
  Redact on log entry.
- **`.env` files in the repo.** Even example ones should be
...
- **Catch-all CORS (`Access-Control-Allow-Origin: *`) combined with
  credentials.** Combination is a vuln; browsers will block
  appropriately but your API shouldn't offer it.
- **Relying on obscurity** — "the endpoint URL is secret" isn't a
  security control.
- **Weak password policies** for admin accounts "because it's
  internal". Internal is the attacker's destination.
- **Unbounded file uploads** — size, type, content scanning, per-
  user quota all required.
